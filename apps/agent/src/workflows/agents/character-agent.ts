...
2. 已有信息的字段按类型处理后再调用 save_field 保存：
   - 标识性字段（name / gender / narrativeRole）照存用户原词，不扩写：姓名只存名字本身（如「林澜」），性别只存性别（如「女」），叙事定位一句话以内（如「主角」）；名字来历、外貌等描述性内容属于其他字段，不得混入；
   - 其余描述性字段以用户描述为种子发散丰富——补充自然贴合的细节、形象与语气，扩写成 2~4 句饱满的角色设定文字；发散限于合理延展，不得与用户给出的事实相悖；
3. 缺失的必填字段：用 ask_user 向用户征集文本，一次只问一个字段，问题要具体（征集到的内容同样按上述规则处理后保存）；
4. save_field 会把丰富后的内容展示给用户确认：用户给出调整意见时，按反馈修正后重新保存该字段；用户放弃则跳过（仅可选字段可跳过）；
...
原则：
- save_field 保存的是「创作再加工」而非原话照抄：在忠于用户核心设定的前提下让内容更丰满立体，短描述扩写成有画面感的设定，已较完整的描述做润色补足；用户确认时可继续调整；
 ...
 * 逐字段「用户输入 → 发散丰富 → 用户确认 → 保存」，必填字段齐全后组装返回角色卡。
 */
export async function createCharacter(
  ...
  const saveField = tool({
    description:
      "把某个字段的内容保存到角色卡。描述性字段（背景/渴望/恐惧/性格/外貌等）以用户描述为种子发散丰富后保存（不照抄原话）；标识性字段（name/gender/narrativeRole）照存用户原词不扩写（姓名只存名字本身、性别只存性别、叙事定位一句话以内）。调用后会展示内容请用户确认；用户要求调整时需按反馈修正后重新保存该字段。",
    inputSchema: z.object({
      field: z.enum(FIELD_NAMES).describe("要保存的字段名"),
      summary: z
        .string()
        .min(1)
        .describe("该字段的保存内容：标识性字段为用户原词；描述性字段为发散丰富后的设定文字（2~4 句，含贴合的细节与形象）"),
    }),
