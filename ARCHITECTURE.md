...
│                         #   argv 选会话模式——无参 = 新建小说全流程，`plan-chapters <novelId> <actNodeId>` = 单幕章节规划
...
│   ├── outline-store.ts    # 大纲树按创作 ID 持久化（add/get/list/update + saveOutlineTree 整树事务
│   │                       #   + saveChapters 幕下批量建章，梗概与关键情节点随节点入库，读写双向 zod 校验）
...
├── electron/
│   ├── main.ts          # 主进程：窗口 + AgentProcess（spawn bun headless——AgentStartOptions
│   │                    #   映射 argv、消息 zod 复验转发、hello 版本校验、退出回收）
│   │                    #   + library 查询 / 管理 IPC（spawn 一次性查询 CLI、shared
│   │                    #   schema 复验、超时兜底）
│   └── preload.ts       # contextBridge 最小 API（start（携带会话参数）/ stop / respond /
│                        #   onMessage / onExit / listNovels / getNovelDetail / rename /
│                        #   setPinned / setFavorite / delete + 诊断钩子读取）
├── src/                 # renderer（React）：App 页面级切换——三栏浏览主页（书库 / 结构树 /
│                        #   预览）↔ 独立创作页（CreationFlow 问答流）↔ 独立章节规划页
│                        #   （ChapterPlanFlow 幕节点「规划本幕章节」问答流）——两会话页共用
│                        #   hooks/use-agent-session（agent 会话泵：消息订阅 / 提问队列 / 重启）
│                        #   + NovelListPanel（右键菜单管理：重命名 / 置顶 / 收藏 / 删除
│                        #   强确认——DeleteNovelDialog 输入小说名放行）/ StructureTreePanel /
│                        #   PreviewPane / OutlineNodeCard（幕卡含规划入口）/ StageBar /
...
客户端幕节点卡「规划本幕章节」入口的会话编排（headless `plan-chapters <novelId> <actNodeId>` 模式，决策见 DECISIONS）：

1. **前置校验**（全部抛可读错误，由协议入口转 fatal error）：小说存在 → 幕节点存在、同小说且类型为 act → 幕梗概与关键情节点非空（旧数据缺内容拒绝）→ 所属部节点存在 → 该幕尚无章节点（同一幕重复规划被唯一索引拒绝，修订流为既有待办）
2. **上下文组装（全取自库）**：novels.name / description（书名 / logline）+ 部名与部梗概 + 幕名 / 幕梗概 / 关键情节点；theme 经 `readOutlineTheme` 从 output 产物尽力读取，缺失省略
3. **复用章节 Agent**：`planChapters`（save_chapters 确认门循环同大纲）→ 确认后 `saveChapters` 幕下事务批量建章 → 通知入库统计；run_finished 携带小说 ID 供客户端刷新
