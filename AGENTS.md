...
- `apps/client/`：`@novel/client`——Electron 桌面客户端（electron-vite 三段式 + React，三栏浏览 + 覆盖层创作）
  ...
  - `src/`：renderer（React）——App 页面级切换：三栏浏览主页（左栏书库可汉堡折叠，右键菜单管理：重命名 / 置顶 / 收藏 / 删除；中栏世界观·角色·大纲结构树——大纲读 outlines 表节点，点击节点右栏按节点展示；右栏预览）↔ 独立创作页（CreationFlow 问答流，整页呈现、头部返回书库）+ NovelListPanel / StructureTreePanel / PreviewPane / OutlineNodeCard 大纲节点卡 / StageBar 六阶段进度 / ViewCard 只读视图 / QuestionCard 五类提问输入
