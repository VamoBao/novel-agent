...
- `packages/shared/`：`@novel/shared`——双端共享的纯 zod 层：领域 schema（worldview / character / location / foreshadow / conflict / outline / audience / outline-node，原 `src/schemas/` 迁出）+ 展示视图 schema（views.ts）+ stdio JSON 协议消息 schema（protocol.ts）+ 库查询结果契约（query.ts）；agent 与客户端共同引用，零运行时依赖
