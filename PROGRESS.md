...
- 2026-09-25 [feat] 新建小说流程收尾于大纲入库，章节规划改为手动逐幕发起（用户直接决策，见 DECISIONS）：createNovel 移除大纲确认后的第一幕章节规划段（`stage("chapter")` / planChapters / saveChapters），收尾通知改为指引用户从书库幕节点「规划本幕章节」入口手动发起；shared stage 枚举与 chapter-plan 视图保留（单幕会话在用，仅注释修正，协议零结构改动）；客户端 StageBar 七阶段 → 六阶段（创作流不再到达「章节」，避免进度条停在未完成态），ChapterPlanFlow 注释同步；create-novel 集成测试删章节段断言（视图序列无 chapter-plan、库中无章节点、新增手动指引通知断言）。AC：typecheck 3 工程 / lint / 166 用例全过（连续 10 轮稳定）；真实 LLM 端到端待用户复验（`bun run dev` 走到大纲确认后会话即结束；客户端新建小说走完大纲 run_finished 回书库，各幕卡显示规划入口）

- 2026-09-25 [test] 修复 foreshadow patch 用例同毫秒时间戳偶发失败：`done.updatedAt > created.createdAt` 断言在 add 与两次 patch 同毫秒落盘时为 false（`new Date().toISOString()` 毫秒精度），add 后隔 20ms 再 patch（对齐 worldview / novel / outline 用例既有惯例——四处均有该防护，唯本用例漏加）。存量问题，与当日流程改动无关（干净 master 全量跑 8/8 复现）。AC：typecheck 3 工程 / lint / 166 用例连续 10 轮全绿

- 2026-09-24 [feat] 幕节点「规划本幕章节」入口——客户端单幕章节规划会话（复杂需求，决策见 DECISIONS）：打通「已有小说 × 任意未规划幕」的章节规划链路。agent 侧 `planActChapters`（workflows/plan-act-chapters.ts）：前置校验五类（小说不存在 / 幕不存在或异小说 / 非幕节点 / 已有章节 / 旧数据缺内容，可读错误）→ 从库组装上下文（书名/logline + 部与幕内容；theme 经 outline-writer 新增 `readOutlineTheme` 尽力读取）→ 复用章节 Agent 确认门 → saveChapters 入库；headless 新增 `plan-chapters <novelId> <actNodeId>` argv 会话模式（无参行为不变，协议 schema 零结构改动，run_finished 语义泛化为一次会话结束）。client 侧：`agent:start` IPC 携带 shared 新增 `AgentStartOptions` 类型映射 spawn 参数；OutlineNodeCard 幕卡「规划本幕章节」入口（已规划幕隐藏 / 旧数据禁用提示 / 部与未接线不渲染）；新 ChapterPlanFlow 独立章节规划页（与 CreationFlow 共用抽取的 useAgentSession 会话泵，单阶段不展示 StageBar；run_finished 后回浏览页刷新详情并保持选中该幕，结构树第三层出现章节点、入口随之消失）。AC：typecheck 3 工程 / lint / 166 用例全过（新增 9：plan-act-chapters 集成 7 + readOutlineTheme 2）；OutlineNodeCard / 两会话页 SSR 冒烟 10 条；headless 用法错误退出 1 + 无效小说 fatal error 实证；真实 LLM 协议级端到端——「万流归序」第二幕规划 5 章确认入库、run_finished、重复规划前置拒绝（可读 fatal error）。GUI 观感待人工验收
...
- 2026-09-14 ~ 2026-09-24【滚动摘要，28 条已归并】早期建设期 26 条（详见 git 历史）：SQLite 持久化从零到体系：characters → worldviews（1:1，taboos JSON）→ novels（全表主键 UUIDv7 化，PRAGMA user_version 管理重建，外键关联）→ outlines 树形多版本表（同父级当前版本 sort 唯一索引），各表配 store 读写双向 zod 校验。交互确认门体系：save_field 逐字段「概括→确认→保存」→ submit_character 整卡确认 → save_outline 大纲确认循环（拒绝→按反馈修订→再确认），并行确认串行化（SerialLineSource）。ReAct 基建：通用运行器（终态工具模式 + 未完成自动续跑）+ ask-user 工厂函数 + AI SDK 7 / deepseek provider 接入。CLI 双模式输入（修复 Bun 管道 readline 丢行）；askInt 范围默认值；可选命名步骤；大纲两级结构（部→幕）对齐 outlines 表接入工作流（先问幕数再问部数，`outlineSchemaFor` refine 强校验恰好 M 部共 N 幕，`saveOutlineTree` 整树事务入库，output JSON 同步两级化）；幕数经 outlineSchemaForActs 在 save_outline 入参层强校验；UUIDv7 切换 Bun 原生 `Bun.randomUUIDv7()`；大纲产物按创作 ID 落盘 `output/`；建立 workflows 模块级状态文档（架构/进度自根文档按模块视角回填，顺带修正根架构文档两处失真）；项目脚手架初始化（Bun + TS6 + ESLint 10 + Agent 文档工作流）；docker-compose 本地查库辅助（sqlite-web，宿主 8081，非运行时依赖）；三份工作流指引与入口文档对齐项目现状（AGENTS.md 项目阶段与入口描述修正、目录清单补登 specs/files/.env.example、两级状态文档约定、git-commit-guide scope 与禁止提交清单、bug-fix-guide 复盘位置对齐）；需求指南对齐 create-agent-docs 新版模板（需求分级工作流：简单快速通道 / 复杂完整流水线、任务清单落盘、WIP=1、清单归一）；移除误入库的 .claude 技能文件（git rm --cached 解除跟踪，本地保留）。各条均含 typecheck/lint/test 与端到端真实 LLM 验证（《灵脉破晓》《九州残脉》《逆脉》《地脉长歌》等）。另含 2026-09-24 [docs] README 全量重写对齐 monorepo 与客户端现状（三工作区一览 / 功能特性 / 快速开始 / 常用命令与环境变量表核实 / 数据存储 / 文档导引，命令与脚本对照 package.json 核实）；2026-09-24 [feat] 伏笔概念数据层——foreshadows 表 + ForeshadowStore 完整 CRUD + foreshadowSchema/PatchSchema + SCHEMA_VERSION 7→8 纯新增表迁移（决策见 DECISIONS，AC：150 用例 + 真实库副本迁移无损，创作流采集/浏览接入为后续需求）
