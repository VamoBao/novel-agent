...
describe("WorldviewStore", () => {
  test("saveWorldview 后 getWorldview 取回且通过 schema 校验（含 taboos JSON 往返）", async () => {
    ...
    const store = WorldviewStore.open(join(tempDir, "test.db"));
    const novelId = "dddddddd-0000-4000-8000-000000000001";
  ...
  test("可选字段缺省时往返仍为 undefined", async () => {
    ...
    const store = WorldviewStore.open(join(dir, "test.db"));
    store.saveWorldview("dddddddd-0000-4000-8000-000000000002", {
      background: { geography: "当代上海" },
      taboos: ["不能出现超自然力量"],
    });
    const stored = store.getWorldview("dddddddd-0000-4000-8000-000000000002");
  ...
  test("重复保存为覆盖更新：单行、内容更新、created_at 保留", async () => {
    const dir = await mkdtemp(join(tmpdir(), "novel-wv-db3-"));
    const store = WorldviewStore.open(join(dir, "test.db"));
    const novelId = "dddddddd-0000-4000-8000-000000000003";
    store.saveWorldview(novelId, makeWorldview("九州大陆"));
  ...
  test("未保存的 ID 返回 undefined", async () => {
    const dir = await mkdtemp(join(tmpdir(), "novel-wv-db4-"));
    const store = WorldviewStore.open(join(dir, "test.db"));
    expect(store.getWorldview("dddddddd-0000-4000-8000-0000000000ff")).toBeUndefined();
    store.close();
    await rm(dir, { recursive: true, force: true });
  });

  test("与 characters 同库不同表，互不影响", async () => {
    const dir = await mkdtemp(join(tmpdir(), "novel-wv-db5-"));
    const dbPath = join(dir, "shared.db");
    const novelId = "dddddddd-0000-4000-8000-000000000005";
    const wv = WorldviewStore.open(dbPath);
